...
class Jugador(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        self.rect.clamp_ip(pygame.Rect(0, 220, W, 660 - 150))
...
class Medusa(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.left < 0 or self.rect.right > W:
            self.velocidad_x *= -1
        if self.rect.top < 220 or self.rect.bottom > H:
            self.velocidad_y *= -1
        self.animation_counter += 1
        if self.animation_counter >= self.animation_speed:
            self.animation_counter = 0
...
class Enemigo(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.x = W  # se genera en el lado derecho de la pantalla
...
def mostrar_temporizador(pantalla, tiempo_transcurrido):
    ...
    texto_temporizador = font.render(f"Tiempo: {tiempo_transcurrido}s", True, blanco)
...
def jugar_nivel():
    ...
    ajustar_volumen(0)
    fondo = pygame.image.load("imagenes/fondofinal2.jpg").convert()
    ...
    tiempo_inicio = temporizador.iniciar_temporizador()    
