...
def game_over(pantalla):
    ...
    # Botón de reiniciar
    boton_reiniciar = Boton("Reiniciar", 1000, 700,
...
def pausar_juego(pantalla):
    ...
    pantalla.blit(texto, (W // 2 - texto.get_width() //
                  2, H // 2 - texto.get_height() // 2))
    pygame.display.update()
    ...
    while True:
        ...
        for enemigo in pygame.sprite.spritecollide(jugador, sprites, False):
            if isinstance(enemigo, Enemigo):
                jugador.vida -= 20  # Disminuir vida de tortuga
        ...
        texto_puntuacion = f"Puntuacion : {puntos}"
        ...
        texto_vida = font_vida.render(
            f"Vida: {jugador.vida}", True, (255, 0, 0))
